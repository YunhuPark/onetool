...
- **Smart Analysis:** 나를 언팔로우한 '👿 배신자(?)'와 나만 팔로우하는 '👼 내 팬(?)' 목록 추출 분리 제공
- **Features:** 오름차순/내림차순 정렬, 실시간 검색, 개별 아이디 리스트 터치 복사, 결과 초기화 기능 제공
...
### 🍲 5. 국밥 화폐 계산기 (Gukbap Currency)
> "지금 내 돈, 진짜 가치는 얼마일까?"
- **Real-time Convert:** 금액을 입력하면 뜨끈한 국밥, 아메리카노, 치킨 등 실물 체감 단위로 실시간 변환
- **Presets:** 자주 입력하는 단위(+1만, +5만 등) 빠르게 추가

### 💥 6. 분노 게이지 샷건 테스트 (Rage Click Test)
> "10초 광클! 당신의 스트레스 지수는?"
- **Mini Game:** 10초 동안 버튼을 최대한 많이, 빨리 누르는 게임
- **Visual Feedback:** 클릭할 때마다 화면 흔들림 및 텍스트 팝업 애니메이션
- **Results:** 타수별로 티어를 나누어 코믹한 결과 제공 (예: 해탈한 부처, 물어뜯기 직전 치와와 등)

### 👽 7. 외계어/MZ 번역기 (Alien & MZ Translator)
> "평범한 글은 지루할 때. 초성, 외계어로 변환해 보세요."
- **Modes:** 2000년대 외계어, 야민정음, 초성, 킹받는 말투 등 기능별 탭 제공
- **Live Output:** 원본 텍스트를 입력하자마자 실시간으로 변환된 텍스트 확인 가능
- **Copy:** 원클릭으로 결과 텍스트를 클립보드로 복사
